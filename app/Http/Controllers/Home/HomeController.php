...
class HomeController extends BaseController
{
    ...
    /**
     * @var \App\Service\GoodsSkuService
     */
    ...
    public function index(Request $request)
    {
        ...
        $banners = Banner::query()
            ->where('is_open', Banner::STATUS_OPEN)
            ->orderBy('ord', 'DESC')
            ->orderBy('id', 'DESC')
            ->get();
        return $this->render('static_pages/home', ['data' => $goods, 'banners' => $banners], __('dujiaoka.page-title.home'));
    }
    ...
    public function buy(int $id)
    {
        try {
            ...
            $this->goodsSkuService->ensureDefaultSku($goods);
            $goods = $this->goodsService->detail($id);
            // 有没有优惠码可以展示
